...
public class MimeMessageUtils {
	...
	/**
	 * Constant indicating a multipart message with a single root multipart element
	 * of type "mixed". Texts, inline elements and attachements will all get added
	 * to that root element.
	 * <p>
	 * This was Spring 1.0's default behavior. It is known to work properly on
	 * Outlook. However, other mail clients tend to misinterpret inline elements as
	 * attachments and/or show attachments inline as well.
	 */
	public static final int MULTIPART_MODE_MIXED = 1;

	/**
	 * Constant indicating a multipart message with a single root multipart element
	 * of type "related". Texts, inline elements and attachements will all get added
	 * to that root element.
	 * <p>
	 ...
	 * properly on Outlook, Outlook Express, Yahoo Mail, and to a large degree also
	 * on Mac Mail (with an additional attachment listed for an inline element,
	 * despite the inline element also shown inline). Does not work properly on
	 * Lotus Notes (attachments won't be shown there).
	 */
	public static final int MULTIPART_MODE_RELATED = 2;

	/**
	 * Constant indicating a multipart message with a root multipart element "mixed"
	 * plus a nested multipart element of type "related". Texts and inline elements
	 * will get added to the nested "related" element, while attachments will get
	 * added to the "mixed" root element.
	 * <p>
	 * This is the default since Spring 1.2.1. This is arguably the most correct
	 * MIME structure, according to the MIME spec: It is known to work properly on
	 * Outlook, Outlook Express, Yahoo Mail, and Lotus Notes. Does not work properly
	 * on Mac Mail. If you target Mac Mail or experience issues with specific mails
	 * on Outlook, consider using MULTIPART_MODE_RELATED instead.
	 */
	...
	/**
	 * Create a new MimeMessageUtils for the given MimeMessage, assuming a simple
	 * text message (no multipart content, i.e. no alternative texts and no inline
	 * elements or attachments).
	 * <p>
	 * The character encoding for the message will be taken from the passed-in
	 * MimeMessage object, if carried there. Else, JavaMail's default encoding will
	 * be used.
	 * 
	...
	/**
	 * Create a new MimeMessageUtils for the given MimeMessage, in multipart mode
	 * (supporting alternative texts, inline elements and attachments) if requested.
	 * <p>
	 * Consider using the MimeMessageUtils constructor that takes a multipartMode
	 ...
	 * 
	 * @param mimeMessage the mime message to work on
	 * @param multipart   whether to create a multipart message that supports
	 ...
	 * @throws MessagingException if multipart creation failed
	 */
	public MimeMessageUtils(MimeMessage mimeMessage, boolean multipart, String encoding) throws MessagingException {
	 ...
	 * 
	 * @param mimeMessage   the mime message to work on
	 * @param multipartMode which kind of multipart message to create (MIXED,
	 ...
	 * @throws MessagingException if multipart creation failed
	 */
	public MimeMessageUtils(MimeMessage mimeMessage, int multipartMode, String encoding) throws MessagingException {
	 ...
	 * (MULTIPART_MODE_MIXED, MULTIPART_MODE_RELATED) or in a nested element rather
	 ...
	 * multipart element will either be added as nested element of type "related"
	 ...
	 * 
	 * @param root the root MimeMultipart object, which attachments will be added
	 *             to; or {@code null} to indicate no multipart at all
	 * @param main the main MimeMultipart object, which text(s) and inline elements
	 *             will be added to (can be the same as the root multipart object,
	 *             or an element nested underneath the root multipart element)
	 */
	 ...
	 * <p>
	 * This will be the direct content of the MimeMessage, in case of a multipart
	 * mail.
	 * 
	 ...
	 * 
	 * @param priority the priority value; typically between 1 (highest) and 5
	 *                 (lowest)
	 ...
	 * body part in multipart mode. Always applies the default content type
	 * "text/plain". else, mail readers might not be able to resolve inline
	...
	/**
	 * Set the given text directly as content in non-multipart mode or as default
	 * body part in multipart mode. The "html" flag determines the content type to
	 * apply. else, mail readers might not be able to resolve inline references
	 * correctly.
	 * 
	 * @param text the text for the message
	...
	/**
	 * Set the given plain text and HTML text as alternatives, offering both options
	 * to the email client. Requires multipart mode. else, mail readers might not be
	 * able to resolve inline references correctly.
	 * 
	 ...
	 * <p>
	 * Note that the InputStream returned by the DataSource implementation needs to
	 * be a <i>fresh one on each call</i>, as JavaMail will invoke
	 * {@code getInputStream()} multiple times.
	 * 
	 * @param attachmentFilename the name of the attachment as it will appear in the
	 *                           mail (the content type will be determined by this)
	 * @param dataSource         the {@code jakarta.activation.DataSource} to take
	 *                           the content from, determining the InputStream and
	 *                           the content type
	 * @throws MessagingException in case of errors
	 */
	 ...
	 * {@code java.io.File}.
	 * <p>
	 * The content type will be determined by the name of the given content file. Do
	 * not use this for temporary files with arbitrary filenames (possibly ending in
	 * ".tmp" or the like)!
	 * 
	 * @param attachmentFilename the name of the attachment as it will appear in the
	 *                           mail
